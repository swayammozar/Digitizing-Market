...
import { formatPrice, mediaUrl, price } from "@/lib/catalog";
import { threadHex, threadLabel } from "@/lib/threadColors";
import { useCart } from "@/lib/cart";
import type { DesignSpecs, Product } from "@/lib/types";

/**
 * The product itself, independent of how it is presented. A Mac window and an
 * iOS full-screen sheet show exactly the same thing; only the chrome around it
 * differs, so the two-pane grid simply collapses to one column on a phone.
 */
export default function ProductDetail({
  ...
  product: Product;
  /** Called after a design goes in the cart, so each shell can react. */
  onAdded?: () => void;
}) {
  ...
  return (
    <div className="grid h-full grid-cols-1 lg:grid-cols-[minmax(0,1.05fr)_minmax(300px,0.95fr)]">
      ...
      <div className="scroll-thin overflow-auto border-black/10 bg-white/30 p-6 lg:border-l">
        ...
        <button
          ...
          disabled={inCart}
          className="w-full rounded-lg bg-[color:var(--color-hanko)] px-4 py-3 text-[14px] font-semibold text-white shadow-sm transition-[filter,transform] hover:brightness-110 active:scale-[.99] disabled:cursor-default disabled:bg-black/10 disabled:text-black/45"
        >
      ...
      <dl className="space-y-2 text-[12.5px]">
        {specs.sizes.map((size, i) => (
          <div key={i} className="flex justify-between gap-4">
            ...
            <dd className="tabular text-right text-[color:var(--label-on-panel)]">
...
function Gallery({ product }: { product: Product }) {
  const shots = [
    ...(product.media.video
      ? [{ kind: "video" as const, src: product.media.video }]
      : []),
  ...
  return (
    <div className="flex min-h-0 flex-col gap-3 p-5">
      <div className="relative aspect-[4/3] min-h-0 overflow-hidden rounded-[var(--radius-panel)] bg-black/[.06] lg:aspect-auto lg:flex-1">
        {current?.kind === "video" ? (
          <video
            key={current.src}
